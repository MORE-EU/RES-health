...
def convert_labels_to_dt(labels, df): 
    """
    Convert numerical labels to datetime format based on dataframe index.

    Args:
        labels: List of numerical labels.
        df: Pandas dataframe containing data with datetime index.

    Returns:
        l: List of datetime strings corresponding to the labels.
           Elements are in the format '%Y-%m-%d'.
           If a label is out of range, the corresponding element is 'None'.
    """
    l = []
    for label in labels:
...
def evaluate_motifs(col,df,df_soil_output,alles_df):
    """
    Evaluate motifs based on various metrics and return evaluation results.

    Args:
        col: Column of the dataframe to perform the analysis.
        df: Pandas dataframe containing the data.
        df_soil_output: Pandas dataframe of soiling events.
        alles_df: Dataframe containing parameters for motif evaluation.

    Returns:
        eval_df: Dataframe with evaluation results including F1, Jaccard, Recall, Hamming, Precision.
        score_list: List of maximum F1 scores for each evaluated motif.
        type_list: List of motif types with the highest F1 scores.
    """
    import sklearn.metrics
...
def matching_eval(col,df_old,df_new,alles_df,events,df_soil_output,n,plot=True):
    """
    Evaluate motifs by matching them against a new dataframe and return evaluation results.

    Args:
        col: Column of the dataframe to perform the analysis.
        df_old: Old dataframe containing the data.
        df_new: New dataframe containing the data to match motifs against.
        alles_df: Dataframe containing parameters for motif evaluation.
        events: Number of events to match.
        df_soil_output: Pandas dataframe of soiling events.
        n: Number of rows from alles_df to evaluate.
        plot: True to display plots, False otherwise.

    Returns:
        teliko: List of maximum F1 scores for each evaluated motif.
        telos: Dataframe with evaluation results including F1, Jaccard, Recall, Hamming, Precision.
    """
...
def match_graph_multi_dim_eval(col,df_new,df_old,df_soil_output,alles_df,n,plot=False):
    """
    Evaluate motifs using multi-dimensional matching and generate plots if requested.
