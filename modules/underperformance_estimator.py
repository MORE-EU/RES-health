...
class UnderperformanceEstimator(ClusterMixin, BaseEstimator):
    """
    An estimator for detecting underperformance clusters in a dataset.

    Parameters:
        params (dict): LightGBM model hyperparameters.
        quantile (float): Quantile parameter for LGBMRegressor (default is 0.9).
        segm_thsh (float): Threshold for segment scoring (default is 0.3).
        chp_thsh (float): Threshold for changepoint detection (default is 0.999).

    Attributes:
        scores (list): List of segment scores.
        chps (pd.Index): Index of detected changepoints.
        res (np.ndarray): Residual values.
        labels_ (np.ndarray): Cluster labels (0 for normal, 1 for underperformance).
    """
    ...
    def fit(self, df_train, y):
        """
        Fit the underperformance estimator to the training data.
        ...
        Returns:
            self (UnderperformanceEstimator): Fitted estimator.
        """
